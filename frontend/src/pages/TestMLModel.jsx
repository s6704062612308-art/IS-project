...
export default function TestMLModel() {
  ...
  const handleGaming = (key, value) => setGaming(prev => ({ ...prev, [key]: value }))
  const handleSalary = (key, value) => setSalary(prev => ({ ...prev, [key]: value }))
  ...
  const predict = async () => {
    ...
    try {
      if (dataset === 'gaming') {
        const res = await axios.post(`${API}/predict/gaming/ensemble`, {
          genre: gaming.genre,
          rank: gaming.rank,
          voice: gaming.voice,
        ...
        const res = await axios.post(`${API}/predict/salary/ensemble`, {
          major: salary.major,
      ...
      setError('ไม่สามารถเชื่อมต่อ Backend ได้ กรุณาตรวจสอบว่า FastAPI รันอยู่ที่ http://localhost:8000')
    }
    setLoading(false)
  }

  const mae = dataset === 'gaming' ? 5 : 3000
  const unit = dataset === 'gaming' ? 'ชั่วโมง' : 'บาท'

  return (
    <div className="container">
      <div className="card">
        <h1>หน้า 3: ทดสอบ Ensemble Machine Learning Model</h1>
        <p>กรอกข้อมูลเพื่อพยากรณ์ผลลัพธ์ด้วย Voting Regressor</p>
      </div>

      <div className="select-dataset">
        <button
          className={dataset === 'gaming' ? 'active' : ''}
          onClick={() => { setDataset('gaming'); setResult(null); setError(null) }}
        >
          Gaming — hours_played_per_week
        </button>
        <button
          className={dataset === 'salary' ? 'active' : ''}
          onClick={() => { setDataset('salary'); setResult(null); setError(null) }}
        >
        ...
        {dataset === 'gaming' ? (
          <div className="form-grid">
            <div className="form-group">
              ...
              <select value={gaming.genre} onChange={e => handleGaming('genre', e.target.value)}>
                {['Sports', 'RPG', 'FPS', 'MOBA'].map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>ระดับ Rank</label>
              <select value={gaming.rank} onChange={e => handleGaming('rank', e.target.value)}>
                {['Gold', 'Silver', 'Bronze', 'Platinum'].map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>การใช้ Voice Chat</label>
              <select value={gaming.voice} onChange={e => handleGaming('voice', e.target.value)}>
                {['Never', 'Often', 'Sometimes'].map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>Platform ที่ใช้</label>
              <select value={gaming.platform} onChange={e => handleGaming('platform', e.target.value)}>
                {['PC', 'Console', 'Mobile'].map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>Season Pass</label>
              <select value={gaming.season_pass} onChange={e => handleGaming('season_pass', e.target.value)}>
                {['Yes', 'No'].map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>สถานะการเล่น</label>
              <select value={gaming.active_status} onChange={e => handleGaming('active_status', e.target.value)}>
                {['Active', 'Inactive'].map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>ยอดซื้อในเกม (บาท)</label>
              <input type="number" min={0} max={10000} value={gaming.purchases}
                onChange={e => handleGaming('purchases', e.target.value)} />
            </div>
            <div className="form-group">
              <label>คะแนนความสำเร็จ</label>
              <input type="number" min={0} max={50000} value={gaming.achievement}
                onChange={e => handleGaming('achievement', e.target.value)} />
            </div>
            <div className="form-group">
              <label>จำนวนแมตช์ที่เล่น</label>
              <input type="number" min={0} max={5000} value={gaming.matches}
                onChange={e => handleGaming('matches', e.target.value)} />
            </div>
            <div className="form-group">
              <label>อัตราชนะ (win rate)</label>
              <input type="number" min={0} max={1} step={0.01} value={gaming.win_rate}
                onChange={e => handleGaming('win_rate', e.target.value)} />
            </div>
            <div className="form-group">
              <label>จำนวนเพื่อนในเกม</label>
              <input type="number" min={0} max={1000} value={gaming.friends}
                onChange={e => handleGaming('friends', e.target.value)} />
            </div>
            <div className="form-group">
              <label>ชั่วโมงสตรีม</label>
              <input type="number" min={0} max={200} value={gaming.streaming}
                onChange={e => handleGaming('streaming', e.target.value)} />
            </div>
            <div className="form-group">
              <label>ค่า Latency (ms)</label>
              <input type="number" min={0} max={500} value={gaming.latency}
                onChange={e => handleGaming('latency', e.target.value)} />
            </div>
            <div className="form-group">
              <label>จำนวน DLC ที่มี</label>
              <input type="number" min={0} max={200} value={gaming.dlc}
          ...
          <div className="form-grid">
            <div className="form-group">
              ...
              <select value={salary.major} onChange={e => handleSalary('major', e.target.value)}>
                {['Software Engineering', 'Data Science', 'Computer Science',
                  'Information Technology', 'Cybersecurity', 'Computer Engineering']
                  .map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>มหาวิทยาลัย</label>
              <select value={salary.university} onChange={e => handleSalary('university', e.target.value)}>
                {['มหาวิทยาลัยอื่น', 'KMUTT', 'เชียงใหม่', 'จุฬาลงกรณ์',
                  'มหิดล', 'ขอนแก่น', 'ธรรมศาสตร์', 'มศว', 'เกษตรศาสตร์']
                  .map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>ตำแหน่งงาน</label>
              <select value={salary.job_role} onChange={e => handleSalary('job_role', e.target.value)}>
                {['UX/UI Designer', 'Software Developer', 'Network Engineer',
                  'Data Analyst', 'DevOps Engineer', 'Data Scientist', 'IT Support']
                  .map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>ขนาดบริษัท</label>
              <select value={salary.company_size} onChange={e => handleSalary('company_size', e.target.value)}>
                {['Startup', 'MNC (บริษัทข้ามชาติ)', 'SME', 'บริษัทใหญ่ไทย']
                  .map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>จังหวัดที่ทำงาน</label>
              <select value={salary.province} onChange={e => handleSalary('province', e.target.value)}>
                {['กรุงเทพ', 'ต่างจังหวัด', 'ปริมณฑล'].map(v => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>เกรดเฉลี่ย (GPA)</label>
              <input type="number" min={1} max={4} step={0.01} value={salary.gpa}
                onChange={e => handleSalary('gpa', e.target.value)} />
            </div>
            <div className="form-group">
              <label>จำนวนครั้งที่ฝึกงาน</label>
              <input type="number" min={0} max={10} value={salary.internship}
                onChange={e => handleSalary('internship', e.target.value)} />
            </div>
            <div className="form-group">
              <label>จำนวน Certification</label>
              <input type="number" min={0} max={20} value={salary.certifications}
                onChange={e => handleSalary('certifications', e.target.value)} />
            </div>
            <div className="form-group">
              <label>คะแนน TOEIC</label>
              <input type="number" min={0} max={990} value={salary.toeic}
                onChange={e => handleSalary('toeic', e.target.value)} />
            </div>
            <div className="form-group">
              <label>จำนวนโปรเจกต์</label>
              <input type="number" min={0} max={50} value={salary.projects}
      ...
      {result && (
        <div>
          <div className="result-box">
            <div className="result-label">ผลการพยากรณ์ (Ensemble Model)</div>
